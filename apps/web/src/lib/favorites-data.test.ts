...
const tree: LocalTreeNode[] = [
  {
    ...
    children: [{ kind: "file", path: "projects/a.md", name: "a.md" }],
  },
...
describe("buildStarredViewData", () => {
  ...
  it("resolves a note row color from its parent folder", () => {
    const view = buildStarredViewData({
      vaultId: VAULT,
      vaultName: VAULT_NAME,
      tree,
    ...
    expect(view.notes[0].colorHex).toBe("#d9f99d");
  });

  it("resolves the leading colorHex and carries the vault label", () => {
    const view = buildStarredViewData({
      vaultId: VAULT,
      vaultName: VAULT_NAME,
      tree,
    ...
    expect(view.folders[0].colorHex).toBe("#d9f99d");
  ...
  it("ignores favorites from other vaults", () => {
    const view = buildStarredViewData({
      ...
      tree,
      favorites: [
        { kind: "note", vaultId: "other", path: "root.md", addedAt: 1 },
      ],
    });
